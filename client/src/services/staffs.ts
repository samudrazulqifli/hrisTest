...
const api = createApi({
  ...
  endpoints: (builder) => ({
    ...
    updateAttendance: builder.query<StaffDetailResponse, string>({
      query: (id) => ({
        url: `/staffs/attendance/${id}`,
      }),
      providesTags: ['Staff'],
    }),
  }),
...
export const { useGetListStaffsQuery, useGetDetailStaffQuery,useUpdateAttendanceQuery, util: exampleUtil } = api
